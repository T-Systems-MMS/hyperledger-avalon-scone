...
function cleanup {
	...
	rm -rf fspf-file
	rm -rf cas-ca.pem
	rm -rf flask_session.yml
	rm -rf redis_session.yml
	rm -rf cacert.pem
...
export FLASK_APP_ALIAS="hospital-app-scone"
...
curl -v -k -s --cert client.pem  --key client-key.pem  --data-binary @redis_session.yml -X POST https://$SCONE_CAS_EXTERNAL_ADDR/session
...
sed -i -e 's/API_DNS/'$FLASK_APP_ALIAS'/g' flask_session.yml

# note: this is insecure - use scone session create instead
curl -v -k -s --cert client.pem  --key client-key.pem  --data-binary @flask_session.yml -X POST https://$SCONE_CAS_EXTERNAL_ADDR/session


# create file with environment variables

cat > myenv << EOF
export FLASK_APP_ALIAS="$FLASK_APP_ALIAS"
export FLASK_SESSION="$FLASK_SESSION"
export REDIS_SESSION="$REDIS_SESSION"
export SCONE_CAS_ADDR="$SCONE_CAS_ADDR"
export AVALON_NETWORK="$AVALON_NETWORK"
export IMAGE="$IMAGE"
