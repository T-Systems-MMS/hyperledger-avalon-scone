...
import receive_request as receive_request
import worker_encryption
import worker_signing
import worker_hash
import workload_processor
from error_code import WorkerError
...
# This method generates key and self signed cert which could be used to upload sessions at CAS
# This step is not part of core worker functionality and many workflows i.e. python or server 
# call might not need certificates
# But for some workloads there may be a need to upload/update session
# Workload developer may use this key and cert however they need
...
class WorkOrderProcessor():
    """
    Scone work order processing class.
    """

# -------------------------------------------------------------------------
    # Class variables

    # Scone SGX TARGET INFO
    #Scone_SGX_TARGET_INFO_FILE = "/dev/attestation/my_target_info"
    # Scone SGX user report data
    #Scone_SGX_USER_REPORT_DATA_FILE = "/dev/attestation/user_report_data"
    # Scone SGX Quote file
    #Scone_SGX_QUOTE_FILE = "/dev/attestation/quote"
...
# -------------------------------------------------------------------------
    ...
    def _generate_worker_keys(self):
        ...
        logger.info("Fetching worker signing and encryption keys")
...
# -------------------------------------------------------------------------
    ...
    def _get_mrenclave(self):
        Get mrenclave value of Scone-SGX worker enclave.

        Returns :
            mrenclave value in case of Scone-SGX worker as hex string.
            If Intel SGX environment is not present returns empty string.
        try:
            # Read target info
            target_info_file = \
                WorkOrderProcessor.Scone_SGX_TARGET_INFO_FILE
            with open(target_info_file, 'rb') as fd:
                data = fd.read()
                mrencalve = data[:32].hex()
        except Exception:
            logger.warn("Scone-SGX Environment not setup."
...
# -------------------------------------------------------------------------
    ...
    def _get_quote(self):
        Get quote of Scone-SGX worker enclave.

        Returns :
            quote value in case of Scone-SGX worker as hex string.
            If Intel SGX environment is not present returns empty string.
        try:
            # Write user report data
            user_report_data_file = \
                WorkOrderProcessor.Scone_SGX_USER_REPORT_DATA_FILE
            ...
            # Read quote
            quote_file = WorkOrderProcessor.Scone_SGX_QUOTE_FILE
