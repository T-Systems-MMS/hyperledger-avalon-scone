...
# Test file to send work orders to Scone python worker
def main(argv):
    if len(argv) >= 1:
    ...
    zmq_url = "tcp://process-work-order-scone:7777"
...
def _create_work_order_request(workload_id, input_data_str,
    ...
    worker_id = hashlib.sha256("scone-hello".encode("UTF-8")).hexdigest()
