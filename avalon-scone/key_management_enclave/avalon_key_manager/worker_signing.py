...
# -------------------------------------------------------------------------
    ...
    def get_private_sign_key(self):
            """
            Get ECDSA Private (Verifiying) key as serialized PEM bytes.
            generate_signing_key() should be called before calling
            this function.

            Returns :
                ECDSA Private (Verifiying) key as serialized PEM bytes.
            """
            if self.sign_private_key:
                return self.sign_private_key.to_pem()
            else:
                return None
